#!/usr/bin/env python3
"""Reference solution for correct DDP data sharding.

Run this program with at least two processes.  It trains a tiny classifier and
records the sample identifiers processed by every rank.  A
``DistributedSampler`` gives each rank a disjoint shard, while ``set_epoch``
changes the deterministic shuffled order between epochs.

Example::

    torchrun --standalone --nproc-per-node=2 solution_data_sharding.py \
        --device cpu --output solution-report.json
"""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Demonstrate correct data sharding in native PyTorch DDP.",
...
def make_dataloader(
    ...
    DistributedSampler[tuple[Tensor, Tensor, int]],
]:
    """Create one deterministic, shuffled dataset shard per DDP rank."""
...
def run(arguments: argparse.Namespace) -> None:
    """Set up DDP, build the input pipeline, and verify the solution."""
    ...
    dataloader, sampler = make_dataloader(
        dataset, arguments.batch_size, arguments.seed, runtime
    )
    ...
    report = {
        "implementation": "distributed_sampler_reference",
    ...
    if not all_epochs_exactly_once:
        raise RuntimeError("SHARDING CHECK failed in the reference solution")
    print("\nSHARDING CHECK: passed; every sample was assigned exactly once")
    if all(rank_order_changed):
        print("RESHUFFLING OBSERVATION: every rank's order changed")
    else:
        print(
            "RESHUFFLING OBSERVATION: at least one rank received the same "
            "order; different random permutations can coincide, especially "
            "for very small datasets"
        )


def main() -> None:
    """Run the solution and always release the process group."""
